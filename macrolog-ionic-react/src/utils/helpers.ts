...
// Ionicon name for each health tier — import from 'ionicons/icons' where used
export const getHealthScoreIconName = (score: number): string => {
  if (score >= 90) return 'leaf-outline';
  if (score >= 70) return 'checkmark-circle-outline';
  if (score >= 50) return 'thumbs-up-outline';
